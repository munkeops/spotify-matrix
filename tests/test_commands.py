...
def test_set_widget_command_applies_local_widget(tmp_path, monkeypatch):
    ...
    response = asyncio.run(commands_module.run_command(CommandRequest(command="set_widget", value="core.clock")))

    saved = config_module.config_service.get_config()
    assert response.ok is True
    assert response.matched is True
    assert response.widgetId == "core.clock"
...
def test_trigger_event_command_uses_display_policy(tmp_path, monkeypatch):
    ...
    assert response.ok is True
    assert response.matched is True
    assert response.widgetId == "core.spotify"
...
def test_trigger_event_command_reports_unmatched_event(tmp_path, monkeypatch):
    ...
    policy_module.display_policy_service.save_policy(DisplayPolicy(mode="single", activeWidgetId="core.clock", triggers=[]))
    ...
    assert response.widgetId is None

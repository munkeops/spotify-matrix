...
6. install, configure, and run the app
...
The runtime entrypoint in `app.toml` points to the Python class exported by `renderer/app.py`:
...
Config fields in `app.toml` become UI controls in Assistant Matrix.
...
Every publishable app needs a static 64x64 preview PNG.

```bash
poetry run assistant-matrix-app preview renderer.app:UserHelloApp --output previews/matrix-64.png --config '{"message":"OK"}'
```
...
```bash
poetry run assistant-matrix-app preview renderer.app:UserHelloApp --output previews/matrix-64.png --config @preview-config.json
```
...
If `card.gif` exists and is declared in `app.toml`, the Store UI uses it first. Otherwise it falls back to `matrix-64.png`.
  ...
  "apps": [
    {
      ...
      "name": "Hello Matrix",
      "version": "0.1.0",
      "summary": "Hello Matrix app for Assistant Matrix.",
      "category": "custom",
      "author": "Assistant Matrix",
      "manifestUrl": "https://store.example.com/apps/user.hello/0.1.0/app.toml",
      "archiveUrl": "https://store.example.com/apps/user.hello/0.1.0/user.hello-0.1.0.tar.gz",
      "previewGifUrl": "",
      "matrixPreviewUrl": "https://store.example.com/apps/user.hello/0.1.0/previews/matrix-64.png",
      "sha256": "..."
    }
  ]
}
...
```text
Settings -> App Store -> Store index URL
```
...
```bash
curl http://<pi-host>:3000/api/apps/store
```

Install the app:

```bash
curl -X POST http://<pi-host>:3000/api/apps/install \
  -H "Content-Type: application/json" \
  -d '{"appId":"user.hello"}'
```

The Pi downloads the archive, validates SHA256 when present, rejects unsafe archive paths and links, checks that `app.toml` matches the store entry, and extracts to:

```text
data/apps/packages/user.hello/
```

Save config:

```bash
curl -X POST http://<pi-host>:3000/api/apps/local/user.hello/config \
  -H "Content-Type: application/json" \
  -d '{"config":{"message":"OK"}}'
```
...
```bash
curl -X POST http://<pi-host>:3000/api/apps/local/user.hello/apply \
  -H "Content-Type: application/json" \
  -d '{"config":{"message":"OK"}}'
```

The runtime restarts in app mode and runs:

```text
spotify_matrix.py --display-mode app --app-dir data/apps/packages/user.hello --app-config data/apps/config/user.hello.json
```
...
curl -X POST http://<pi-host>:3000/api/display/policy \
  ...
  -d '{
    "policy": {
      ...
      "rotation": [
        {"appId": "core.weather", "durationSeconds": 60, "enabled": true},
        {"appId": "user.hello", "durationSeconds": 30, "enabled": true}
      ],
      "triggers": []
    }
  }'
...
Before publishing a app publicly, confirm:

- `app.id` is stable and namespaced, for example `munkeops.weather`.
- `app.version` changes whenever the archive changes.
